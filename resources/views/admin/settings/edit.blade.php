...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold tracking-tight mt-2">Edit Setting</h1>
    </div>
        ...
        <form method="POST" action="{{ route('admin.settings.update', $setting) }}">
            ...
            <div class="mb-5">
                ...
                <textarea id="value" name="value" rows="3"
                    class="w-full rounded-lg bg-gray-800 border border-gray-700 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors placeholder-gray-500 font-mono">{{ old('value', $setting->value) }}</textarea>
            ...
            <div class="mb-5">
                ...
                <textarea id="description" name="description" rows="2"
                    class="w-full rounded-lg bg-gray-800 border border-gray-700 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-colors placeholder-gray-500">{{ old('description', $setting->description) }}</textarea>
                @error('description') <p class="mt-1 text-xs text-red-400">{{ $message }}</p> @enderror
            </div>

            <button type="submit" class="px-6 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-sm font-medium transition-colors">Update Setting</button>
        </form>
    </div>
@endsection
