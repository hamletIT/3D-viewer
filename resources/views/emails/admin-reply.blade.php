...
<body>
    <div class="wrapper">
        <div class="header">
            ...
            <h1>New Reply</h1>
            <p>An admin has replied to your question</p>
        </div>

        <div class="card">
            <h2>Subject</h2>
            <p style="font-size:1rem;font-weight:600;color:#f1f5f9;">{{ $conversation->subject }}</p>
            <h2 style="margin-top:1rem;">Reply</h2>
            <div class="message-box">{{ $conversation->messages()->latest()->first()?->message ?? '—' }}</div>
        </div>

        <div style="text-align:center;">
            <a href="{{ route('user.dashboard', ['tab' => 'conversations']) }}" class="btn">View in Dashboard</a>
        </div>
